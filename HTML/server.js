...
const masterZikrList = [
    ...
    // Font forced to Arabic (False) as requested
...
io.on('connection', (socket) => {
    ...
    socket.on('joinSession', (sessionId) => {
        if (sessions[sessionId]) {
            ...
            socket.emit('error', "Session Not Found. Please refresh.");
        }
    });
    ...
    socket.on('increment', (sessionId) => {
        ...
        if (!session) {
            // FIX: Notify client if session is dead
            socket.emit('sessionError', "Connection lost. Please refresh to rejoin.");
            return;
        }
