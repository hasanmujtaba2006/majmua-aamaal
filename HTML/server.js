    ...
    try {
        if (fs.existsSync(SESSIONS_FILE)) {
            ...
            for (const id in loaded) {
                loaded[id].users = [];
                loaded[id].adminId = null;
                loaded[id].timeout = null;
            }
            console.log(`Loaded ${Object.keys(loaded).length} persistent sessions.`);
            return loaded;
        }
    } catch (err) {
    ...
    try {
        ...
        for (const id in sessions) {
            ...
            toSave[id] = {
                ...
                currentZikrIndex: s.currentZikrIndex,
...
function broadcastSessionState(sessionId) {
    if (pendingBroadcasts[sessionId]) {
        clearTimeout(pendingBroadcasts[sessionId]);
        delete pendingBroadcasts[sessionId];
    }
...
io.on('connection', (socket) => {
    ...
    socket.on('joinSession', ({ sessionId, password }) => {
        ...
        if (session.users.length >= 50) { socket.emit('sessionError', 'This session is full (maximum 50 participants).'); return; }
        socket.join(sessionId);
    ...
    socket.on('reJoinSession', ({ sessionId, adminToken }) => {
        ...
        if (session) {
            ...
            socket.emit('joinedSession', { 
                sessionId: sessionId, 
                isAdmin: isAdmin, 
                adminToken: isAdmin ? session.adminToken : undefined,
    ...
    socket.on('increment', (sessionId) => {
        ...
        if (zikr.type === 'count' && session.currentCount < zikr.target) {
            ...
            if (session.currentCount >= zikr.target) {
                broadcastSessionState(sessionId);
            } else {
                if (!pendingBroadcasts[sessionId]) {
                    pendingBroadcasts[sessionId] = setTimeout(() => {
                        broadcastSessionState(sessionId);
                    }, 200);
                }
            }
