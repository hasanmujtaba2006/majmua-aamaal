...
io.on('connection', (socket) => {
    ...
    socket.on('createSession', ({ name, password, forcedId }) => {
        ...
        sessions[sessionId] = {
            ...
            currentZikrIndex: 0, currentCount: 0, users: [socket.id], timeout: null // Added timeout tracker
        };
    ...
    socket.on('reJoinSession', ({ sessionId, adminToken }) => {
        ...
        if (session) {
            ...
            if(isAdmin) {
                ...
                // Admin returned! Clear the 10-minute timeout so it doesn't expire
                if (session.timeout) {
    ...
    // --- NEW: Leave Session (For normal participants) ---
    ...
    socket.on('disconnect', () => { 
        ...
        // --- NEW: 10 Minute Admin Auto-Expire Logic ---
        for (const sessionId in sessions) {
            const session = sessions[sessionId];
            
            // Remove user from active tracking list
            session.users = session.users.filter(id => id !== socket.id);

            // If the disconnected user was the admin of this session
            if (session.adminId === socket.id) {
                // Set a 10-minute (600,000 ms) timer to destroy the room
